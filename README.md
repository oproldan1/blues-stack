...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create blues-stack-da85
  fly apps create blues-stack-da85-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app blues-stack-da85
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app blues-stack-da85-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'blues-stack-da85-staging' does not match app name in config file 'blues-stack-da85'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name blues-stack-da85-db
  fly postgres attach --app blues-stack-da85 blues-stack-da85-db

  fly postgres create --name blues-stack-da85-staging-db
  fly postgres attach --app blues-stack-da85-staging blues-stack-da85-staging-db
  ```
